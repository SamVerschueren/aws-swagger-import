...
/**
 * Constructor method for APIGateway
 * @param  {object}  options  - AWS sdk class options. Can contain region, profile and filename
 */
function APIGateway(options) {
	AWS.config.region = options.region || 'us-west-1';
	...
	if (options.filename) {
		credentialsOptions.filename = options.filename;
	}
